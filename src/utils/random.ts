...
export function hashString(seed: string) {
  return cyrb128(seed)[0] >>> 0;
}

export function hashToUnitFloat(seed: string) {
  return hashString(seed) / 4294967295;
}

export function hashCombine(...parts: Array<string | number>) {
  return parts.join('|');
}

export function seededRange(seed: string, min: number, max: number) {
  return min + (max - min) * hashToUnitFloat(seed);
}
