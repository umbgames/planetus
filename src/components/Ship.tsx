...
export function Ship({
  ...
  const navEulerRef = useRef(new THREE.Euler(0, 0, 0, 'YXZ'));
  const navFrameThrottle = useRef(0);

  const lastFired = useRef(0);
  const lastMissile = useRef(0);

  const [explosions, setExplosions] = useState<
    { id: string; pos: THREE.Vector3; createdAt: number; size: number }[]
  >([]);
  const [muzzleFlashes, setMuzzleFlashes] = useState<
    { id: string; pos: THREE.Vector3; createdAt: number }[]
  >([]);

  const publishNavState = (elapsedTime: number) => {
    const activeCamera = cameraRef.current ?? (camera as THREE.PerspectiveCamera);
    navEulerRef.current.setFromQuaternion(activeCamera.quaternion, 'YXZ');

    setNavState({
      navShipPosition: position.current.clone(),
      navShipYaw: navEulerRef.current.y,
      navCameraPitch: navEulerRef.current.x,
      navTime: elapsedTime,
      navVisible: currentPlanetId !== null,
    });
  };

  const getPlanetPos = (id: string | null, elapsedTime: number) => {
    if (!id || !solarSystem) return new THREE.Vector3(0, 0, 0);
    const planet = solarSystem.bodies.find(
      (b): b is PlanetData => b.type === 'planet' && b.id === id
    );
    if (!planet) return new THREE.Vector3(0, 0, 0);
    const pos = getPlanetWorldPosition(planet, elapsedTime, orbitMap);
  ...
  useFrame((state, delta) => {
    ...
    const currentPlanetPos = getPlanetPos(currentPlanetId ?? null, elapsedTime);
    ...
    if (!isJumping && solarSystem && frameCount.current % 30 === 0) {
      ...
      for (const body of solarSystem.bodies) {
        if (body.type === 'planet' && body.id !== currentPlanetId) {
          const planetPos = getPlanetPos(body.id, elapsedTime);
          const distToPlanet = position.current.distanceTo(planetPos);

          const orbitThreshold = Math.max(
            (body as PlanetData).radius * VISUAL_SCALE.PLANET_RADIUS_MULTIPLIER * 6.5,
            18
          );
      ...
      if (!switched && currentPlanetId !== null) {
        ...
        const currentPlanet = solarSystem.bodies.find(
          (b): b is PlanetData => b.type === 'planet' && b.id === currentPlanetId
        );

        const currentPlanetThreshold = currentPlanet
          ? Math.max(currentPlanet.radius * VISUAL_SCALE.PLANET_RADIUS_MULTIPLIER * 10, 30)
          : 30;

        if (
          distToSun < solarSystem.starRadius * VISUAL_SCALE.STAR_RADIUS_MULTIPLIER * 1.35 &&
          position.current.length() < currentPlanetThreshold
        ) {
          if (setCurrentPlanetId) {
    ...
    if (isJumping && lockedTarget?.type === 'planet' && solarSystem) {
      ...
      if (targetPlanet) {
        const targetPlanetPos = getPlanetPos(targetPlanet.id, elapsedTime);
        ...
        if (distToTarget < (targetPlanet as PlanetData).radius * 5) {
          ...
          const jumpDir = new THREE.Vector3()
            .subVectors(targetPlanetPos, position.current)
            .normalize();
        ...
        navFrameThrottle.current += delta;
        if (navFrameThrottle.current >= 1 / 30) {
          navFrameThrottle.current = 0;
          publishNavState(elapsedTime);
        }
    ...
    if (!isLocked && !isLaunching && !isMobile) {
      navFrameThrottle.current += delta;
      if (navFrameThrottle.current >= 1 / 30) {
        navFrameThrottle.current = 0;
        publishNavState(elapsedTime);
    ...
    if (keys['KeyT'] || mobileKeys.lock) {
        ...
        if (dist < 50) {
          ...
          if (dot > 0.8) {
            ...
            if (score > bestScore) {
              ...
              bestTarget = {
                ...base,
                position: basePos.clone().add(currentPlanetPos),
                type: 'base',
              };
            }
          }
        ...
        if (dist < 50) {
          ...
          if (dot > 0.8) {
            ...
            if (score > bestScore) {
              ...
              bestTarget = {
                ...sat,
                position: satPos.clone().add(currentPlanetPos),
                type: 'satellite',
                health: 100,
              };
            }
      ...
      activePlayers.forEach((player) => {
        ...
        const playerPos = new THREE.Vector3(
          player.playerSave.position.x,
          player.playerSave.position.y,
          player.playerSave.position.z
        ).add(currentPlanetPos);
    ...
    if (lockedTarget && lockedTarget.type === 'satellite') {
      const angle = elapsedTime * lockedTarget.speed + lockedTarget.initialAngle;
      const satPos = new THREE.Vector3(lockedTarget.orbitRadius, 0, 0);
      satPos.applyAxisAngle(new THREE.Vector3(0, 1, 0), angle);
      satPos.applyEuler(
        new THREE.Euler(lockedTarget.tiltX, lockedTarget.tiltY, lockedTarget.tiltZ)
      );
      satPos.add(currentPlanetPos);

      setLockedTarget((prev: any) => (prev ? { ...prev, position: satPos } : null));
    } else if (lockedTarget && lockedTarget.type === 'base') {
      const updatedBase = bases.find((b) => b.id === lockedTarget.id);
      if (updatedBase) {
        const basePos = new THREE.Vector3(
          updatedBase.position.x,
          updatedBase.position.y,
          updatedBase.position.z
        ).add(currentPlanetPos);

        setLockedTarget((prev: any) =>
          prev ? { ...prev, position: basePos, health: updatedBase.health } : null
        );
      }
      ...
      if (updatedPlayer && updatedPlayer.playerSave) {
        const playerPos = new THREE.Vector3(
          updatedPlayer.playerSave.position.x,
          updatedPlayer.playerSave.position.y,
          updatedPlayer.playerSave.position.z
        ).add(currentPlanetPos);

        setLockedTarget((prev: any) =>
          prev ? { ...prev, position: playerPos, shipConfig: updatedPlayer.shipConfig } : null
        );
      } else {
        setLockedTarget(null);
      }
    } else if (lockedTarget && lockedTarget.type === 'planet' && solarSystem) {
      const targetPlanetPos = getPlanetPos(lockedTarget.id, elapsedTime);

      if (!lockedTarget.position || !lockedTarget.position.equals(targetPlanetPos)) {
        setLockedTarget((prev: any) => (prev ? { ...prev, position: targetPlanetPos } : null));
      }
    }
    ...
    if ((keys['MouseLeft'] || mobileKeys.mg) && now - lastFired.current > 100) {
      if (!userData || userData.machineGunAmmo > 0) {
        ...
        const rightOffset = new THREE.Vector3(0.5, 0, 0).applyQuaternion(
          shipRef.current!.quaternion
        );
        const leftOffset = new THREE.Vector3(-0.5, 0, 0).applyQuaternion(
          shipRef.current!.quaternion
        );
        const forwardOffset = new THREE.Vector3(0, 0, -1).applyQuaternion(
          shipRef.current!.quaternion
        );
    ...
    setProjectiles((prev) => {
      ...
      prev.forEach((p) => {
        ...
        activePlayers.forEach((player) => {
          if (hit || player.uid === userData?.uid || player.currentPlanetId !== currentPlanetId)
            return;
          if (!player.playerSave) return;
          ...
          if (p.pos.distanceTo(playerPos) < 2.0) {
            ...
            if (userData) {
              gameManager
                .damagePlayer(player.uid, damage)
                .then((isDead) => {
                  if (isDead) {
    ...
    if (dist < minHeight) {
      ...
      const upAfterClamp = localPos.clone().normalize();
      const downwardVelocity = velocity.current.clone().projectOnVector(upAfterClamp);

      if (downwardVelocity.dot(upAfterClamp) < 0) {
  ...
  return (
    <>
      ...
      {(() => {
        ...
        const isOwnBase =
          lockedTarget.type === 'base' &&
          userData &&
          lockedTarget.ownerId === userData.uid;
