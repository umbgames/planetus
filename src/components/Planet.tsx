...
const Clouds = memo(function Clouds({ radius, isMobile = false }: CloudsProps) {
  ...
  useFrame((state) => {
    ...
    if (cloudsRef.current) {
      cloudsRef.current.rotation.y = t * 0.01;
      cloudsRef.current.rotation.z = t * 0.005;
    }
  });
...
export const Planet = memo(function Planet({
  ...
  const planetSegments = useMemo(() => {
    ...
    // These values are much safer while still looking good.
  ...
  return (
    <group>
      ...
      <Clouds radius={radius} isMobile={isMobile} />
